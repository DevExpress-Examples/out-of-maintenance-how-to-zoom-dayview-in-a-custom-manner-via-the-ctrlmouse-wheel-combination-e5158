...
Namespace DXApplication5
    Public NotInheritable Class DataHelper
        ...
        Public Shared Sub GenerateEvents(ByVal eventList As AppointmentCollection, ByVal count As Integer, ByVal collection As ResourceCollection, ByVal storage As SchedulerStorage)
        ...
        Public Shared Sub GenerateEventsWithSharedResource(ByVal eventList As AppointmentCollection, ByVal count As Integer, ByVal collection As ResourceCollection, ByVal storage As SchedulerStorage)
            For i As Integer = 0 To count - 1
        ...
        Private Shared Function AddSharedResources(ByVal subject As String, ByVal status As Integer, ByVal label As Integer, ByVal collection As ResourceCollection, ByVal storage As SchedulerStorage) As Appointment
            ...
            apt.Start = Date.Today.AddDays(1) + TimeSpan.FromMinutes(rnd.Next(0, rangeInMinutes))
        ...
        Private Shared Function CreateEvent(ByVal subject As String, ByVal resourceId As Object, ByVal status As Integer, ByVal label As Integer, ByVal storage As SchedulerStorage) As Appointment
            Dim apt As Appointment = storage.CreateAppointment(AppointmentType.Normal)
            apt.Subject = subject
            ...
            Dim rnd As New Random()
            Dim rangeInMinutes As Integer = 60 * 24
            apt.Start = Date.Today + TimeSpan.FromMinutes(rnd.Next(0, rangeInMinutes))
            apt.End = apt.Start + TimeSpan.FromMinutes(rnd.Next(0, rangeInMinutes \ 4))
            apt.StatusKey = status
            apt.LabelKey = label
            Return apt
        ...
        Public Shared Sub AddRecurrentAppointment(ByVal eventList As AppointmentCollection, ByVal storage As SchedulerStorage)
            ...
            aptPattern.RecurrenceInfo.Start = Date.Now.AddDays(2)
